...
#include "ut_creportersendalldialogplugin.h"
...
// Unit test.
void Ut_CReporterSendAllDialogPlugin::init()
{
...
void Ut_CReporterSendAllDialogPlugin::initTestCase()
{
}

void Ut_CReporterSendAllDialogPlugin::cleanup()
{
    if (m_Server != 0) {
        delete m_Server;
        m_Server = 0;
...
void Ut_CReporterSendAllDialogPlugin::cleanupTestCase()
{
}
...
void Ut_CReporterSendAllDialogPlugin::testDestroyPlugin()
{
...
void Ut_CReporterSendAllDialogPlugin::testRequestPluginInvalidArguments()
{
    // Test request rejected, when arguments are not valid.
    m_Subject->initialize(m_Server);

    bool retVal = m_Subject->requestDialog(QVariantList());
    QVERIFY(retVal == false);

}

void Ut_CReporterSendAllDialogPlugin::testRequestHandledFilesSent()
{
    ...
    int button = static_cast<int>(CReporter::SendAllButton);
...
void Ut_CReporterSendAllDialogPlugin::testRequestHandledFilesDeleted()
{
    // Test successfull request handling - Delete all.
    QSignalSpy requestCompletedSpy(m_Subject, SIGNAL(requestCompleted()));
    ...
    // Fake "Send" -button clicked from dialog.
    int button = static_cast<int>(CReporter::DeleteAllButton);
    m_Subject->actionPerformed(button);
...
void Ut_CReporterSendAllDialogPlugin::testRejectDialog()
{
...
QTEST_APPLESS_MAIN(Ut_CReporterSendAllDialogPlugin)
